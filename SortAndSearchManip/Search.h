...
namespace SortAndSearchManip
{
	...
	SORT_AND_SEARCH_MANIP_DLLEXPORT std::pair<int, int> FindIndicesOfSum(const std::vector<int>& v, int sum, bool UseSTL = true);

	// Returns the index "i" into "values" such that (v[0]+...+v[i-1] == v[i+1] +...+v[v.size()-1])
	// Returns -1 if no such index exists.
	SORT_AND_SEARCH_MANIP_DLLEXPORT int FindSumMidPointIndex(const std::vector<int>& values);

	// Given a circularly sorted vector of values, returns the number of times the vector
	// has been rotated.
	// values must not have any duplicates.
	SORT_AND_SEARCH_MANIP_DLLEXPORT int FindNumTimesRotated(const std::vector<int>& values);

	// Given a circularly sorted vector of values, returns the index of the specified value 'x'.
	// values must not have any duplicates.
	SORT_AND_SEARCH_MANIP_DLLEXPORT int FindValueInCircularlySortedArray(const std::vector<int>& values, int x);
}
